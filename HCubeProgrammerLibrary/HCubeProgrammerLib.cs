...
public class HCubeProgrammerLib
{
    public HCubeProgrammerLib()
    {
        ...
        if (stLinkList != IntPtr.Zero)
        {
            ...
            for (var i = 0; i < x1; i++)
            {
                var currentItem = Marshal.PtrToStructure<debugConnectParameters>(stLinkList + (i * size));
        ...
        GetLLastError(out EXCEPTION lastException);
        
    }

    [DllImport("D:\\6 Develope\\HCubeProgrammer\\STM32CubeProgrammerAPI\\lib\\CubeProgrammerAPI.dll", CallingConvention = CallingConvention.Cdecl)]
    ...
    [DllImport("D:\\6 Develope\\HCubeProgrammer\\STM32CubeProgrammerAPI\\lib\\CubeProgrammerAPI.dll", CallingConvention = CallingConvention.Cdecl)]
    public static extern void GetLLastError(out EXCEPTION lastException);
